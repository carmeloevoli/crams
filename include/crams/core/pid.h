#ifndef CRAMS_CORE_PID_H_
#define CRAMS_CORE_PID_H_
...
class PID {
 public:
  ...
  constexpr int getZ() const { return m_Z; }
  ...
  constexpr bool isH() const { return m_Z == 1; }
  ...
  friend std::ostream& operator<<(std::ostream& stream, const PID& pid) {
    if (pid.isTertiary())
      stream << "(" << pid.m_A << "," << pid.m_Z << ",tertiary)";
    else
      stream << "(" << pid.m_A << "," << pid.m_Z << ")";
    return stream;
  }
