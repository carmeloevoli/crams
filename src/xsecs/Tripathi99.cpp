...
double get_wilson_rms_radius(const int& A) {
  const double factor = std::sqrt(5. / 3.) * CRAMS::CGS::fm;
  double radius;
  if (A > 26) {
...
double inelastic_sigma(int A_p, int Z_p, int A_t, int Z_t, double T_n) {
  assert(A_p > 0 && A_t > 0);
  const double r_0 = 1.1 * CRAMS::CGS::fm;
  double f_A = pow2(A_p) + pow2(A_t) + 2 * A_p * A_t * (1. + T_n / CRAMS::CGS::protonMassC2);
  double E_cm_mev = CRAMS::CGS::protonMassC2 * std::sqrt(f_A);
  E_cm_mev /= CRAMS::CGS::MeV;
  double Tn_mev = T_n / CRAMS::CGS::MeV;
  ...
  double Radius = (r_p + r_t) / CRAMS::CGS::fm;
